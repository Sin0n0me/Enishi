        ...
        void text(std::string& value, std::uint8_t encoding) {
            ...
            while (this->position < end && this->error.empty()) {
                ...
                if (unit > 0xD7FF && unit < 0xDC00) {
                    ...
                    std::uint16_t low{};
                    this->read(low);
                    if (!this->require(low > 0xDBFF && low < 0xE000, "invalid surrogate pair")) {
                        return;
                    }
        ...
        foundation::Result<PMXData, AssetError> parse() {
            ...
            this->records(data.bones, BONE_MINIMUM_SIZE, [&](PMXBone& v) {
                ...
                if ((v.flags & BONE_FLAG_IK) != 0) {
                    ...
                    this->require(v.ik_iterations > -1 && v.ik_angle > -1, "invalid IK settings");
            ...
            this->records(data.display_frames, DISPLAY_FRAME_MINIMUM_SIZE, [&](PMXDisplayFrame& v) {
                names(v);
                this->read(v.special);
            ...
            if (data.version == PMX_VERSION_2_1) {
                ...
                this->records(data.soft_bodies, SOFT_BODY_MINIMUM_SIZE, [&](PMXSoftBody& v) {
                    ...
                    this->require(v.shape < 2 && v.group < 16 && v.link_distance > -1 &&
                                      v.cluster_count > -1 && v.aero_model > -1 && v.aero_model < 5,
