    ...
    class OpenGLResourceAccessor final : public platform::IRenderResourceAccessor,
      ...
      public:
        ...
        types::HandleId make_render_target_view(
            const types::HandleId&, RTV&& view) noexcept override;
        types::HandleId make_shader_resource_view(
            const types::HandleId&, SRV&& view) noexcept override;
        types::HandleId make_depth_stencil_view(
            const types::HandleId&, DSV&& view) noexcept override;
        types::HandleId make_unordered_access_view(
            const types::HandleId&, UAV&& view) noexcept override;
        foundation::Option<RTV&> get_render_target_view(types::HandleId) noexcept override;
        foundation::Option<const RTV&> get_render_target_view(
            types::HandleId) const noexcept override;
        foundation::Option<SRV&> get_shader_resource_view(types::HandleId) noexcept override;
        foundation::Option<const SRV&> get_shader_resource_view(
            types::HandleId) const noexcept override;
        foundation::Option<DSV&> get_depth_stencil_view(types::HandleId) noexcept override;
        foundation::Option<const DSV&> get_depth_stencil_view(
            types::HandleId) const noexcept override;
        foundation::Option<UAV&> get_unordered_access_view(types::HandleId) noexcept override;
        foundation::Option<const UAV&> get_unordered_access_view(
            types::HandleId) const noexcept override;
        std::span<const RTV> get_render_target_views(void) const noexcept override {
            return this->render_target_list;
        }
        std::span<const SRV> get_shader_resource_views(void) const noexcept override {
            return this->shader_resource_list;
        }
        std::span<const DSV> get_depth_stencil_views(void) const noexcept override {
            return this->depth_stencil_list;
        }
        std::span<const UAV> get_unordered_access_views(void) const noexcept override {
            return this->unordered_access_list;
        }
        std::tuple<types::HandleId, platform::IBufferAccessor::Buffer&> make_buffer(
            void) noexcept override;
        void add_interface(types::HandleId handle,
            const platform::IBufferAccessor::Buffer buffer) noexcept override;
        void remove_buffer(types::HandleId handle) noexcept override {
            this->buffers.erase(handle);
        }
        foundation::Option<platform::IBufferAccessor::Buffer&> get_buffer(
