...
namespace enishi::collider {
    class Collider {
      ...
      public:
        [[nodiscard]] bool register_model(const types::CollisionModelId model);
        bool remove_model(const types::CollisionModelId model);

        // Vertices are bone-local; transform maps that space into world space.
        // Invalid input leaves an existing bone unchanged.
        [[nodiscard]] bool set_bone(const types::CollisionModelId model,
            const types::BoneIndex bone,
            const std::vector<glm::vec3>& positions,
            const glm::mat4& transform = glm::mat4(1.0f));
        [[nodiscard]] bool update_bone(const types::CollisionModelId model,
            const types::BoneIndex bone,
            const glm::mat4& transform);
        bool remove_bone(const types::CollisionModelId model, const types::BoneIndex bone);
        [[nodiscard]] const OBBMap* get_obb_map(const types::CollisionModelId model) const noexcept;

        [[nodiscard]] bool add_handler(const types::CollisionModelId model,
            const std::shared_ptr<sub_system::ICollisionHandler>& handler);
        bool remove_handler(const types::CollisionModelId model,
            const std::shared_ptr<sub_system::ICollisionHandler>& handler);
        ...
        [[nodiscard]] static foundation::Option<types::OBBContact> intersect(
